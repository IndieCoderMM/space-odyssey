...
class Ship < Image
  ...
  def initialize(width: 80, height: 60)
    ...
    img_path = "#{IMG_PATH}#{rand(1..5)}.png"
  ...
  def out_of_bound?
    self.x < -width || self.x > WIN_WIDTH || self.y < -height || self.y > WIN_HEIGHT
  end

  def locate_random
    self.x = rand(width..WIN_WIDTH - width)
    self.y = rand(height..WIN_HEIGHT - height)
  end
